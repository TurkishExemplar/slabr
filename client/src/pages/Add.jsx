...
export default function Add() {
  ...
  // ── Scan state ───────────────────────────────────────────────────────────
  // scanState: 'checking' | 'unavailable' | 'idle' | 'scanning' | 'result' | 'error'
  const [scanState,    setScanState]    = useState('checking');
  const [scanItemType, setScanItemType] = useState('');       // '' = auto-detect
  ...
  // Reset scan to idle when switching away and back
  useEffect(() => {
    if (activeTab === 'scan' && scanState !== 'checking') {
      // Only reset to idle if we were showing a result/error from a previous attempt
      // (Don't reset 'unavailable' — that's permanent until env changes)
    }
  }, [activeTab]);
      ...
      try {
        const res = await fetch('/api/scan', {
          ...
          body: JSON.stringify({
            ...
            item_type:    scanItemType || undefined,
          }),
        });
  ...
  async function handleScanConfirm(e) {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        setScanSubmitError(d.error || 'Failed to add item');
        return;
      }
      navigate('/dashboard');
    } catch {
  ...
  function openForm(item) {
    ...
    setFormError('');
    // Double rAF so the browser paints the element before we trigger transition
      ...
      <main className="max-w-3xl mx-auto px-6 py-8">
          ...
          <div className="space-y-4">
            ...
            <div className="relative">
              ...
              <input
                ...
                placeholder="Search by card name, set, sport, publisher…"
                autoFocus
            ...
            {!query.trim() && (
              <div className="text-center py-12 text-zinc-600 text-sm">
                Type at least one character to search the catalog
              </div>
            )}
          </div>
        )}
        ...
        {activeTab === 'scan' && (
          <div>
            ...
            {scanState === 'unavailable' && (
              <div className="border border-dashed border-zinc-800 rounded-2xl flex flex-col items-center justify-center py-16 gap-4 text-center">
                ...
                <div>
                  ...
                  <p className="text-zinc-600 text-sm mt-1">XIMILAR_API_TOKEN is not configured on this server</p>
                </div>
            ...
            {/* Result — identified card + editable form */}
              ...
              <div className="space-y-5">
                ...
                <div className="bg-zinc-900 border border-zinc-800 rounded-2xl p-5">
                  ...
                  {scanResult.current_value != null && (
                    <div className="mt-3 pt-3 border-t border-zinc-800 flex items-center justify-between">
                      <span className="text-zinc-500 text-xs">Ximilar value</span>
                ...
                <form onSubmit={handleScanConfirm} className="space-y-4">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                      <label className="block text-zinc-500 text-xs mb-1.5">Name</label>
                      <input type="text" value={scanForm.name} onChange={setScanField('name')}
                        className={INPUT} />
                    </div>
                    <div>
                      <label className="block text-zinc-500 text-xs mb-1.5">Set</label>
                      <input type="text" value={scanForm.set_name} onChange={setScanField('set_name')}
                        className={INPUT} />
                    </div>
                    <div>
                      <label className="block text-zinc-500 text-xs mb-1.5">Year</label>
                      <input type="number" value={scanForm.year} onChange={setScanField('year')}
                        className={INPUT} />
                    </div>
                    <div>
                      <label className="block text-zinc-500 text-xs mb-1.5">Card Number</label>
                      <input type="text" value={scanForm.card_number} onChange={setScanField('card_number')}
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                    <div>
                      ...
                      <input type="number" min="1" value={scanForm.quantity} onChange={setScanField('quantity')}
                        className={INPUT} />
                    </div>
                    <div>
            ...
            <div className="flex items-start justify-between px-6 py-5 border-b border-zinc-800 shrink-0">
              <div className="flex-1 min-w-0 pr-3">
                <div className="flex items-center gap-2 mb-1">
                  <span className={`text-[10px] font-semibold px-2 py-0.5 rounded-full uppercase tracking-wider ${TYPE_BADGE[selectedItem.item_type] ?? ''}`}>
                    {CATEGORY_LABELS[selectedItem.item_type]}
                  </span>
                ...
                <>
                  <div>
                    ...
                    <select
                      ...
                      onChange={setField('grading_company')}
                      className={INPUT + ' appearance-none'}
                    >
